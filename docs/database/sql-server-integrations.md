---
title: Connect an ASP.NET Core app to SQL Server using Aspire and Entity Framework Core
description: Learn how to connect an ASP.NET Core app to to SQL Server using Aspire and Entity Framework Core.
ms.date: 12/02/2024
ms.topic: tutorial
---
...
1. On the **Additional information** screen:
    ...
    - Check the **Enlist in Aspire orchestration** option and select **Create**.
...
## Add the Aspire integration to the Blazor app

Add the [Aspire Entity Framework Core Sql Server library package](sql-server-entity-framework-integration.md) to your _AspireSQLEFCore_ project:

```dotnetcli
dotnet add package Aspire.Microsoft.EntityFrameworkCore.SqlServer
```

Your _AspireSQLEFCore_ project is now set up to use Aspire integrations. Here's the updated _AspireSQLEFCore.csproj_ file:

:::code language="xml" source="snippets/tutorial/AspireSQLEFCore/AspireSQLEFCore/AspireSQLEFCore.csproj" highlight="10":::

## Configure the Aspire integration

In the _:::no-loc text="Program.cs":::_ file of the _AspireSQLEFCore_ project, add a call to the <xref:Microsoft.Extensions.Hosting.AspireSqlServerEFCoreSqlClientExtensions.AddSqlServerDbContext%2A> extension method after the creation of the `builder` but before the call to `AddServiceDefaults`. For more information, see [Aspire service defaults](../fundamentals/service-defaults.md). Provide the name of your connection string as a parameter.
...
Add the [Aspire Hosting Sql Server](sql-server-entity-framework-integration.md#hosting-integration) NuGet package to your _AspireStorage.AppHost_ project:
...
1. Select the run button at the top of Visual Studio (or <kbd>F5</kbd>) to launch your Aspire project dashboard in the browser.
1. On the projects page, in the **AspireSQLEFCore** row, click the link in the **Endpoints** column to open the UI of your app.

    :::image type="content" source="media/app-home-screen.png" lightbox="media/app-home-screen.png" alt-text="A screenshot showing the home page of the Aspire support application.":::
...
1. Close the web browser tabs that display the **AspireSQL** web app and the Aspire dashboard.
...
1. In the Aspire dashboard, on the projects page, in the **AspireSQLEFCore** row, click the link in the **Endpoints** column to open the UI of your app.
