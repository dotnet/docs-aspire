...
The PostgreSQL hosting integration models a PostgreSQL server as the <xref:Aspire.Hosting.ApplicationModel.PostgresServerResource> type. To access this type and APIs that allow you to add it to your [📦 Aspire.Hosting.PostgreSQL) NuGet package in the [app host](xref:dotnet/aspire/app-host](https://www.nuget.org/packages/Aspire.Hosting.PostgreSQL) NuGet package in the [app host](xref:dotnet/aspire/app-host) project.
