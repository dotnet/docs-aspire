...
On the previous example, azd takes the bicep resource that was generated by Aspire and set the input parameters for it:
...
Note how `azd` automatically generates a Key Vault for the postgres module to securely persist the connection string using a secret. When the project is deployed to a container app, the secret is used to set the full connection string as a container app secret (see picture above).

If the secret is referenced directly, the container app secret will be a reference to the Key Vault Secret. See below.
...
Note that `connectionstring--server` holds the reference for the Key Vault secret. The secret is pulled by container app when it is used by the service project. On the other hand, the `connectionstring--db2` was created pulling the `connectionString` secret from Key Vault and adding the `Database=` configuration.
